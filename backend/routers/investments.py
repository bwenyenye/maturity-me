...
async def get_investments_rankings():
    try:
        df = data_processor.read_excel_file(settings.INVESTMENTS_RANKINGS_PATH)
        return data_processor.process_rankings(df)
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))

@router.get("/indicators", response_model=List[IndicatorData])
async def get_investments_indicators():
    try:
        df = data_processor.read_excel_file(settings.INVESTMENTS_INDICATORS_PATH)
...
async def get_investments_data():
    try:
        rankings_df = data_processor.read_excel_file(settings.INVESTMENTS_RANKINGS_PATH)
        indicators_df = data_processor.read_excel_file(settings.INVESTMENTS_INDICATORS_PATH)

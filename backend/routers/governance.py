...
async def get_governance_rankings():
    try:
        df = data_processor.read_excel_file(settings.GOVERNANCE_RANKINGS_PATH)
...
async def get_governance_indicators():
    try:
        df = data_processor.read_excel_file(settings.GOVERNANCE_INDICATORS_PATH)
...
async def get_governance_data():
    try:
        rankings_df = data_processor.read_excel_file(settings.GOVERNANCE_RANKINGS_PATH)
        indicators_df = data_processor.read_excel_file(settings.GOVERNANCE_INDICATORS_PATH)

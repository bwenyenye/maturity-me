...
async def get_digital_skills_rankings():
    try:
        df = data_processor.read_excel_file(settings.DIGITAL_SKILLS_RANKINGS_PATH)
        return data_processor.process_rankings(df)
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))

@router.get("/indicators", response_model=List[IndicatorData])
async def get_digital_skills_indicators():
    try:
        df = data_processor.read_excel_file(settings.DIGITAL_SKILLS_INDICATORS_PATH)
        return data_processor.process_indicators(df)
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))

@router.get("/", response_model=PillarResponse)
async def get_digital_skills_data():
    try:
        rankings_df = data_processor.read_excel_file(settings.DIGITAL_SKILLS_RANKINGS_PATH)
        indicators_df = data_processor.read_excel_file(settings.DIGITAL_SKILLS_INDICATORS_PATH)

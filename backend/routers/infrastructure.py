...
async def get_infrastructure_rankings():
    try:
        df = data_processor.read_excel_file(settings.INFRASTRUCTURE_RANKINGS_PATH)
...
async def get_infrastructure_indicators():
    try:
        df = data_processor.read_excel_file(settings.INFRASTRUCTURE_INDICATORS_PATH)
...
async def get_infrastructure_data():
    try:
        rankings_df = data_processor.read_excel_file(settings.INFRASTRUCTURE_RANKINGS_PATH)
        indicators_df = data_processor.read_excel_file(settings.INFRASTRUCTURE_INDICATORS_PATH)
